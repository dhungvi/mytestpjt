...
# In the following we execute all batch files, and keep a list in the @batch_files array
	...
	for ( my $errorRate = 4 ; $errorRate <= 4; $errorRate += 0.25) {
	    print PFILE "Starting jobs: gene $gene_count, error rate $errorRate\n";
	    my $rate = $errorRate / 100;
	    
	    for ( my $i = 0 ; $i < $numOfTrials ; $i++ ) {
		my @batch_info = ($i,$errorRate,$geneID,$len,$numOfEst);
		unless ($processed_jobs{join "\t", @batch_info}) {
		    my $uniqueId = "$$\_$geneID\_$errorRate\_$i";
		    my $executable = "module load cap3; module load bioperl; ./singleTestC++.pl $geneFile $numOfEst $rate $uniqueId $useQualityFile"; 
		    
		    # Set up readhawk execution       
		    my $file = create_batch_file(file => "singleTestC++.$uniqueId", executable => $executable);
...
#Open file and print comments (as before)
	...
	print PFILE "Processing file: $file $id\n";
	wait_on_job($file, $id);
...
# Not sure what these were for, but they must be left out of new script.
