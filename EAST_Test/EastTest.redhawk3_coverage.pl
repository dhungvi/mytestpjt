...
my $geneFile;
my $geneSource = "all_zf_cdna.fa";
my $num_genes = $ARGV[0];
my $minGeneLen = $ARGV[1];
my $maxGeneLen = $ARGV[2];
my $errorRate = $ARGV[3];	#an integer, For example, 4 represents 4% error rate.
my $useQualityFile = $ARGV[4]; #1-use; 0-not use.
my $numOfTrials = 30;
...
foreach my $geneSeq (@gene_list) {
	...
	for ( my $i = 0 ; $i < $numOfTrials ; $i++ ) {
	    ...
	    push(@batch_files, [$file,$id,$i,$coverage,$geneID,$len,$numOfEst]);
...
my $output_file = "resultsOnCoverage\_$num_genes\_$minGeneLen\_$maxGeneLen\_$yyyymmdd";
...
my $output = join "\t", qw(geneID geneLength numberOfEsts Coverage Cap3NumContigs EastNumContigs Cap3NumSing EastNumSin Cap3Len EastLen Cap3AScore EastAScore Cap3Time EastTime Cap3NumUsedEsts EastNumUsedEsts);
print OFILE "#number_of_runs=$numOfTrials\tErrorRate=$errorRate\tuse_quality_file=$useQualityFile\n#Date:$yyyymmdd\n$output\n";

foreach my $p (@batch_files) {
    my ($file, $id, $trial, $coverage, $geneID, $geneLen, $numEST) = @$p;
    ...
    if (@results == 12) {
	print OFILE join "\t", ($geneID, $geneLen, $numEST, $coverage);
	print OFILE "\t", $output;
	print OFILE "\n";
    }
    close($fh);
    ...
    my @input = <$efh>;
    if (@input) {
	print EFILE "gene: $geneID, Error: $coverage, trial: $trial\n";
	print EFILE join "\n", @input;
	print EFILE "\n";
    }
